...
void PIOS_Board_Init(void) {
	...
	if (boot_count < 3) {
		...
		HwPixracerSetDefaults(HwPixracerHandle(), 0);
	...
	HwPixracerUSB_VCPPortGet(&hw_usb_vcpport);

	if (!usb_cdc_present) {
		/* Force VCP port function to disabled if we haven't advertised VCP in our USB descriptor */
		hw_usb_vcpport = HWPIXRACER_USB_VCPPORT_DISABLED;
	}
	...
	HwPixracerUSB_HIDPortGet(&hw_usb_hidport);

	if (!usb_hid_present) {
		/* Force HID port function to disabled if we haven't advertised HID in our USB descriptor */
		hw_usb_hidport = HWPIXRACER_USB_HIDPORT_DISABLED;
	}
	...
	HwPixracerDSMxModeOptions hw_DSMxMode;
	HwPixracerDSMxModeGet(&hw_DSMxMode);

	/* UART1 Port */
	uint8_t hw_uart1;
	HwPixracerUart1Get(&hw_uart1);
	...
	/* UART7 Port */
	uint8_t hw_uart7;
	HwPixracerUart7Get(&hw_uart7);

	PIOS_HAL_ConfigurePort(hw_uart7,             // port type protocol
			&pios_usart7_cfg,                    // usart_port_cfg
			&pios_usart_com_driver,              // com_driver
			NULL,                                // i2c_id
			NULL,                                // i2c_cfg
			NULL,                                // ppm_cfg
			NULL,                                // pwm_cfg
			PIOS_LED_ALARM,                      // led_id
			&pios_usart7_dsm_aux_cfg,            // dsm_cfg
			hw_DSMxMode,                         // dsm_mode
			NULL);                               // sbus_cfg
			
	/* UART8 Port */
	uint8_t hw_uart8;
	HwPixracerUart7Get(&hw_uart8);

	PIOS_HAL_ConfigurePort(hw_uart8,             // port type protocol
			&pios_usart8_cfg,                    // usart_port_cfg
			&pios_usart_com_driver,              // com_driver
			NULL,                                // i2c_id
			NULL,                                // i2c_cfg
			NULL,                                // ppm_cfg
			NULL,                                // pwm_cfg
			PIOS_LED_ALARM,                      // led_id
			&pios_usart8_dsm_aux_cfg,            // dsm_cfg
			hw_DSMxMode,                         // dsm_mode
			NULL);                               // sbus_cfg

	/* Configure the inport */
	uint8_t hw_inport;
	HwPixracerInPortGet(&hw_inport);

	switch (hw_inport) {
	case HWPIXRACER_INPORT_DISABLED:
		break;
	
	case HWPIXRACER_INPORT_PWM:
	...
	case HWPIXRACER_INPORT_WS2811SERIALPPMADC:
		...
		PIOS_HAL_ConfigurePort(HWSHARED_PORTTYPES_PPM,  // port type protocol
				...
				0,                                      // dsm_mode
				NULL);                                  // sbus_cfg
	case HWPIXRACER_INPORT_PPMSERIAL:
	case HWPIXRACER_INPORT_PPMSERIALADC:
	case HWPIXRACER_INPORT_SERIAL:
		{
			uint8_t hw_inportserial;
			HwPixracerInPortSerialGet(&hw_inportserial);
		...
		if (hw_inport == HWPIXRACER_INPORT_SERIAL)
			break;

		if (hw_inport == HWPIXRACER_INPORT_WS2811SERIALPPMADC)
			break;

		// Else fall through to set up PPM.

	case HWPIXRACER_INPORT_PPM:
	case HWPIXRACER_INPORT_PPMADC:
	case HWPIXRACER_INPORT_PPMOUTPUTS:
	case HWPIXRACER_INPORT_PPMOUTPUTSADC:
		PIOS_HAL_ConfigurePort(HWSHARED_PORTTYPES_PPM,  // port type protocol
				NULL,                                   // usart_port_cfg
				NULL,                                   // com_driver
				NULL,                                   // i2c_id
	...
	case HWPIXRACER_INPORT_PPMPWMADC:
		PIOS_HAL_ConfigurePort(HWSHARED_PORTTYPES_PPM,  // port type protocol
				NULL,                                   // usart_port_cfg
				NULL,                                   // com_driver
				NULL,                                   // i2c_id
...
#endif
			break;
		case HWPIXRACER_INPORT_OUTPUTS:
#ifdef PIOS_INCLUDE_SERVO
...
#endif
			break;
		case HWPIXRACER_INPORT_OUTPUTSADC:
#ifdef PIOS_INCLUDE_SERVO
...
/* init sensor queue registration */
	...
	HwPixracerGyroRangeOptions hw_gyro_range;
	HwPixracerGyroRangeGet(&hw_gyro_range);
	switch(hw_gyro_range) {
		case HWPIXRACER_GYRORANGE_250:
			PIOS_MPU_SetGyroRange(PIOS_MPU_SCALE_250_DEG);
			break;
		case HWPIXRACER_GYRORANGE_500:
			PIOS_MPU_SetGyroRange(PIOS_MPU_SCALE_500_DEG);
			break;
		case HWPIXRACER_GYRORANGE_1000:
			PIOS_MPU_SetGyroRange(PIOS_MPU_SCALE_1000_DEG);
			break;
		case HWPIXRACER_GYRORANGE_2000:
			PIOS_MPU_SetGyroRange(PIOS_MPU_SCALE_2000_DEG);
			break;
	}

	HwPixracerAccelRangeOptions hw_accel_range;
	HwPixracerAccelRangeGet(&hw_accel_range);
	switch(hw_accel_range) {
		case HWPIXRACER_ACCELRANGE_2G:
			PIOS_MPU_SetAccelRange(PIOS_MPU_SCALE_2G);
			break;
		case HWPIXRACER_ACCELRANGE_4G:
			PIOS_MPU_SetAccelRange(PIOS_MPU_SCALE_4G);
			break;
		case HWPIXRACER_ACCELRANGE_8G:
			PIOS_MPU_SetAccelRange(PIOS_MPU_SCALE_8G);
			break;
		case HWPIXRACER_ACCELRANGE_16G:
			PIOS_MPU_SetAccelRange(PIOS_MPU_SCALE_16G);
			break;
	}

	// the filter has to be set before rate else divisor calculation will fail
	HwPixracerMPU6000DLPFOptions hw_mpu_dlpf;
	HwPixracerMPU6000DLPFGet(&hw_mpu_dlpf);
	uint16_t bandwidth = \
		(hw_mpu_dlpf == HWPIXRACER_MPU6000DLPF_188) ? 188 : \
		(hw_mpu_dlpf == HWPIXRACER_MPU6000DLPF_98)  ? 98  : \
		(hw_mpu_dlpf == HWPIXRACER_MPU6000DLPF_42)  ? 42  : \
		(hw_mpu_dlpf == HWPIXRACER_MPU6000DLPF_20)  ? 20  : \
		(hw_mpu_dlpf == HWPIXRACER_MPU6000DLPF_10)  ? 10  : \
		(hw_mpu_dlpf == HWPIXRACER_MPU6000DLPF_5)   ? 5   : \
		188;
	...
	HwPixracerMagnetometerGet(&hw_magnetometer);

	switch (hw_magnetometer) {
		case HWPIXRACER_MAGNETOMETER_NONE:
			break;

		case HWPIXRACER_MAGNETOMETER_INTERNAL:
		...
		default:
		{
			HwPixracerExtMagPortOptions hw_mag_port;
			HwPixracerExtMagPortGet(&hw_mag_port);

			pios_i2c_t i2c_mag_id = NULL;
			const void *i2c_mag_cfg = NULL;
			switch (hw_mag_port) {
			case HWPIXRACER_EXTMAGPORT_UART3:
				break;
			case HWPIXRACER_EXTMAGPORT_UART1:
			...
			if (i2c_mag_id && i2c_mag_cfg) {
				...
				HwPixracerExtMagOrientationGet(&hw_orientation);
	...
	if (hw_inport == HWPIXRACER_INPORT_OUTPUTSADC ||
		hw_inport == HWPIXRACER_INPORT_PPMADC ||
		hw_inport == HWPIXRACER_INPORT_PPMOUTPUTSADC ||
		hw_inport == HWPIXRACER_INPORT_PPMPWMADC ||
		hw_inport == HWPIXRACER_INPORT_PPMSERIALADC ||
		hw_inport == HWPIXRACER_INPORT_PWMADC ||
		hw_inport == HWPIXRACER_INPORT_WS2811SERIALPPMADC)
	{
