...
int32_t transmitter_control_update()
{
	...
	for(uint8_t n = 0; n < MANUALCONTROLCOMMAND_CHANNEL_NUMELEM; ++n)
	...
	for (uint8_t n = 0;
		 ...
		 ++n)
	{
		cmd.Channel[n] = PIOS_RCVR_Read(pios_rcvr_group_map[MANUALCONTROLSETTINGS_CHANNELGROUPS_PPM],
				settings.ChannelNumber[n]);

		// If a channel has timed out this is not valid data and we shouldn't update anything
		// until we decide to go to failsafe
		if(cmd.Channel[n] == (uint16_t) PIOS_RCVR_TIMEOUT) {
			//JlinkRTTPrintf(0, "Receiver timeout for channel: %ld\n",n);
	...
	//SEGGER_RTT_WriteString(0, "transmitter updated\n");
	...
	// {
	if(validChannel[1] && validChannel[2])
	{
		ActuatorDesiredData actuatorDesired;
		actuatorDesired.Steering = cmd.Channel[1];
		actuatorDesired.Throttle = cmd.Channel[2];		
		actuatorDesired.Mode = cmd.Channel[4]/1500;
	...
	else if (valid_input_detected)
	{
		...
		cmd.ArmSwitch = scaledChannel[MANUALCONTROLSETTINGS_CHANNELGROUPS_ARMING] > 0 ? MANUALCONTROLCOMMAND_ARMSWITCH_ARMED : MANUALCONTROLCOMMAND_ARMSWITCH_DISARMED;
...
static bool arming_position(ManualControlCommandData *cmd, ManualControlSettingsData *settings)
{
	...
	case MANUALCONTROLSETTINGS_ARMING_CORNERSTHROTTLE:
		return lowThrottle && ((cmd->Yaw > ARMED_THRESHOLD || cmd->Yaw < -ARMED_THRESHOLD) &&
...
static void process_transmitter_events(ManualControlCommandData *cmd, ManualControlSettingsData *settings, bool valid)
{
	...
	*/
	...
	case ARM_STATE_ARMING:
	{
		...
		bool arm = arming_position(cmd, settings) && valid;
		uint16_t arm_time = (settings->ArmTime == MANUALCONTROLSETTINGS_ARMTIME_250) ? 250 : (settings->ArmTime == MANUALCONTROLSETTINGS_ARMTIME_500) ? 500 : (settings->ArmTime == MANUALCONTROLSETTINGS_ARMTIME_1000) ? 1000 : (settings->ArmTime == MANUALCONTROLSETTINGS_ARMTIME_2000) ? 2000 : 1000;
	...
	{
		bool disarm = disarming_position(cmd, settings) && valid;
		uint16_t disarm_time = (settings->DisarmTime == MANUALCONTROLSETTINGS_DISARMTIME_250) ? 250 : (settings->DisarmTime == MANUALCONTROLSETTINGS_DISARMTIME_500) ? 500 : (settings->DisarmTime == MANUALCONTROLSETTINGS_DISARMTIME_1000) ? 1000 : (settings->DisarmTime == MANUALCONTROLSETTINGS_DISARMTIME_2000) ? 2000 : 1000;
...
static void update_stabilization_desired(ManualControlCommandData *manual_control_command, ManualControlSettingsData *settings, SystemSettingsAirframeTypeOptions *airframe_type)
{
	...
	if (*airframe_type != SYSTEMSETTINGS_AIRFRAMETYPE_HELICP && stabilization.Thrust < 0)
		stabilization.Thrust = -1;
...
{
	if (deadband < 0.0001f)
		return; /* ignore tiny deadband value */
	if (deadband >= 0.85f)
		return; /* reject nonsensical db values */
